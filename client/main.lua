...
RegisterNetEvent('AG_CountyAlive:ShowGlobalAlert')
AddEventHandler('AG_CountyAlive:ShowGlobalAlert', function(text)
	if Config.AlertsUseNotification then
		ShowNotification(text)
	end
	if Config.AlertsUseHelpMessage then
		ShowHelpNotification(text, false, 5000)
	end
end)
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.HospitalHealCoords) <= 1.0 do
				...
				DrawText3D(Config.HospitalHealCoords, "Press ~g~[E]~s~ to heal", 0.6)
				if IsControlJustReleased(0, 51) then
					local playerPed = PlayerPedId()
					local maxHealth = GetEntityMaxHealth(playerPed)
					SetEntityHealth(playerPed, maxHealth)
					ShowNotification('~g~You have been healed!')
				end
			end
		end
	end)
		...
		while true do
			...
			for k,v in pairs(Config.FoodVendors) do
				...
				while #(GetEntityCoords(PlayerPedId()) - markerlocation) <= 1.0 do
					Citizen.Wait(0)
					DrawText3D(markerlocation, "Press ~y~[E]~s~ to purchase items", 0.6)
					if IsControlJustReleased(0, 51) then
						-- Open new food menu here
					end
				end
			end
		end
...
if Config.EnableCamping then
	if not Config.UseCommands then
		RegisterCommand(Config.CampingMenuCommand, function()
			-- Open Camping Menus with the following options:
	...
	while true do
		...
		if tentSet then
			...
			while #(GetEntityCoords(tent) - GetEntityCoords(PlayerPedId())) < 2.0 and editMode and not tentLocked do
				...
				ShowHelpNotification('~INPUT_VEH_FLY_PITCH_UD~ : Move Up & Down ~n~~INPUT_CELLPHONE_UP~ ~INPUT_CELLPHONE_LEFT~ ~INPUT_CELLPHONE_RIGHT~ ~INPUT_CELLPHONE_DOWN~ : Move ~n~~INPUT_VEH_FLY_SELECT_TARGET_LEFT~ ~INPUT_VEH_FLY_SELECT_TARGET_RIGHT~ : Rotate ~n~~INPUT_CELLPHONE_EXTRA_OPTION~ : Lock Object', false, 100)
			...
			while #(GetEntityCoords(tent) - GetEntityCoords(PlayerPedId())) < 2.0 and editMode and tentLocked do
				Citizen.Wait(0)
				local hoistCoords = GetEntityCoords(tent)
				ShowHelpNotification('~INPUT_CELLPHONE_EXTRA_OPTION~ : Unlock Object', false, 100)
				if IsControlJustReleased(0, 179) then
					tentLocked = false
				end
			end
		...
		while true do
			...
			for k,v in pairs(Config.FishingLocations) do
				...
				while #(GetEntityCoords(PlayerPedId()) - markerlocation) <= 1.0 do		
					if isFishing then
						Citizen.Wait(0)
						DrawText3D(markerlocation, "Press ~y~[E]~s~ to stop fishing", 0.6)
						if IsControlJustReleased(0, 51) then
						...
						Citizen.Wait(0)
						DrawText3D(markerlocation, "Press ~y~[E]~s~ to start fishing", 0.6)
						if IsControlJustReleased(0, 51) then
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			for k,v in pairs(Config.RentalVehicles) do
				...
				while #(GetEntityCoords(PlayerPedId()) - markerlocation) <= 1.0 do
					Citizen.Wait(0)
					DrawText3D(markerlocation, "Press ~o~[E]~s~ to rent vehicle", 0.6)
					if IsControlJustReleased(0, 51) then
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.RentalVehicleReturnCoord) <= 3.0 do
				Citizen.Wait(0)
				ShowHelpNotification('Press ~b~E~w~ to return the vehicle', false, 100)
...
RegisterNetEvent('AG_CountyAlive:RentVehicleReturn')
AddEventHandler('AG_CountyAlive:RentVehicleReturn', function(vehicleName)
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.TrailSignInCoord) <= 3.0 do
				Citizen.Wait(0)
				DrawText3D(Config.TrailSignInCoord, "Press ~o~[E]~s~ to begin your hike!", 0.6)
				if IsControlJustReleased(0, 51) then
					if not isHiking then
						TriggerServerEvent('AG_CountyAlive:GlobalAlert', Config.HikingAlertText)
						isHiking = true
					else
						ShowNotification('~r~[ERROR]~w~ You have already began your hike!')
					end
				end
			end
		end
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.TrailEndCoords) <= 3.0 do
				Citizen.Wait(0)
				DrawText3D(Config.TrailEndCoords, "Press ~o~[E]~s~ to end your hike!", 0.6)
				if IsControlJustReleased(0, 51) then
					if isHiking then
						...
						ShowNotification('Your hike has ended! Congratulations!')
						isHiking = false
					else
						ShowNotification('~r~[ERROR]~w~ You have not began your hike!')
					end
				end
			end
		end
